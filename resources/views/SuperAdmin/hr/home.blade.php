...
@endif
        ...
        <div class="float-right pr-5 pt-5 pb-5">
            <a href="{{url('/application-from')}}" class="btn btn-primary">New Application</a>
        </div>
  ...
  <tr >
    ...
    <tr >
        ...
        <td>{{$inquery->section}}</td>
        <td>{{$inquery->status}}</td>
        <td>

            <form action="{{url('/add/staff')}}" >
                @csrf
                <input type="number" hidden='true' value="{{$inquery->id}}" name="id">
                <button type="submit" class="btn btn-primary">Interview</button>
            </form>
        </td>
    </tr>
@endforeach
